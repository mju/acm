 ...
 * lhs and rhs should point to different locations
 */
void
 ...
 * target should not be an alias of either lhs or rhs
 */
void
...
main(int argc, char** argv) {
  struct so_long facs[N + 1];
  struct so_long tmp;
  int i;

  make_it_so_long(facs + 1, 1);
  for (i = 2; i < N + 1; i++) {
    make_it_so_long(&tmp, i);
    intact_mul(facs + i, facs + i - 1, &tmp);
  }

  struct so_long sols[N + 1];
  make_it_so_long(sols, 1);
  make_it_so_long(sols + 1, 1);

  int j;
  int a_half;
  for (i = 2; i < N + 1; i++) {
    a_half = (i - 1) >> 1;
    make_it_so_long(sols + i, 0);
    for (j = 0; j < a_half; j++) {
      intact_mul(&tmp, sols + j, sols + i - 1 - j);
      in_place_add(sols + i, &tmp);
    }
    if (a_half == i - 1 - a_half) {
      make_copy(&tmp, sols + i);
      in_place_add(sols + i, &tmp);
      intact_mul(&tmp, sols + a_half, sols + a_half);
      in_place_add(sols + i, &tmp);
    } else {
      intact_mul(&tmp, sols + a_half, sols + i - 1 - a_half);
      in_place_add(sols + i, &tmp);
      make_copy(&tmp, sols + i);
      in_place_add(sols + i, &tmp);
    }
